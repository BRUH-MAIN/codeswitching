...
| **D10** | Train_T2 = 22h | **see Gate 1** | The synthetic sentences are shorter than the paper's, so the corpus falls short of 22h unless `--n-calls` is raised. Train_T1 (8h) is unaffected; M7 is the row at risk, because an 8h-vs-12h contrast is far weaker than the paper's 8h-vs-22h. Gate 1 now prints projected hours and warns below 16h. |
...
| **D13** | 4,466 bigram calls | **15,000** | The 26B repeats itself: 4,466 calls gave 44,713 raw bigrams (paper: 44,657) but only **3,458 unique** vs the paper's 5,932 — 7.7% dedup survival against their 13.3%. Fewer distinct switch points directly weakens CBA, which measures switch-point recognition. Calls are drawn from a seeded RNG, so raising the count re-uses every cached call and pays only for new ones. |
